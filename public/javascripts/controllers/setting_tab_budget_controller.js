...
kakeibo.controller.SettingTabBudgetController.prototype.setDate = function(date){
	if(kakeibo.utils.isValidDate(date) && date.month != 0){
		...
		$th2.html($th2.attr("data-template").replace("%y", before_date.year).replace("%m", before_date.month));
		$th3.html($th3.attr("data-template").replace("%y", before_date.year).replace("%m", before_date.month));
		$th4.html($th4.attr("data-template").replace("%y", date.year).replace("%m", date.month));
...
kakeibo.controller.SettingTabBudgetController.prototype.submit = function(){
	...
	for(var i = 0; i < $tr_list.length; i++){
		...
		if(amount != null){
			...
			jError($("#setting-pane-msg-input-error").attr("data-msg"), {clickOverlay: true});
			return;
		}
	}
