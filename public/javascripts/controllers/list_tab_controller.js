...
kakeibo.controller.ListTabController.prototype.addSearchButton = function(search_param, title){
	...
	var $btn_container = $("#list-pane-all-entries-button").parent().clone();
	var $btn = $btn_container.contents();
	$btn.attr("id", btn_id);

	if(typeof title === 'undefined'){
		$btn.html($("#list-pane-search-entries-button-template").html().replace("%n", ""+this.m_search_button_num));
	}
	else{
		$btn.html(title);
	}
...
kakeibo.controller.ListTabController.prototype.initGrid = function(){
	...
	$("#list-pane-remove-button").click(function(){
		...
		if(id_list.length > 0){
			var msg = $("#list-pane-msg-remove").attr("data-msg").replace("%d", id_list.length);
			var ret = confirm(msg);
			if(ret == true){
				self.m_grid.unselectRow();
